...
def validar_extraccion(datos):
    ...
    if faltantes:
        avisos.append("No encontrados en el reporte (quedaron en 0): " + ", ".join(faltantes) + ".")
...
def extraer_datos_pdf(archivo_bytes):
    """Extrae composición desde bytes de un PDF de laboratorio."""
...
# Matriz de usos industriales de caliza
...
def evaluar_perfiles(valores):
    """Evalúa los 17 perfiles industriales. valores: dict campo -> float | None."""
    resultados = []
...
def calcular_evaluacion(caco3, cao, mgo, sio2, fe2o3, al2o3, so3,
    ...
    """Calcula módulos, fases de Bogue y dictámenes normativos."""
    ...
    if mgo > 5.0:
        cumple_norma = False
        errores_norma.append(f"MgO ({mgo}%): Supera el límite de 5.0%. Evita la expansión perjudicial en el cemento endurecido.")
    if so3 > 3.5:
        cumple_norma = False
        errores_norma.append(f"SO3 ({so3}%): Supera el límite de 3.5%. Controla el tiempo de fraguado y evita expansiones tardías.")
    elif so3 < 3.0:
        advertencias_geol.append(f"SO3 ({so3}%) está por debajo del mínimo recomendado (3.0%). Controla el tiempo de fraguado.")
    if loi > 3.0:
        cumple_norma = False
        errores_norma.append(f"LOI ({loi}%): Supera el límite de 3.0%. Indica una posible prehidratación o carbonatación indeseada.")
    if res_insol > 0.75:
        cumple_norma = False
        errores_norma.append(f"Residuo Insoluble ({res_insol}%): Supera el límite de 0.75%. Control de impurezas.")
    if alcalis > 0.6:
        cumple_norma = False
        errores_norma.append(f"Álcalis ({alcalis}%): Supera el límite de 0.6%. Previene la reacción álcali-agregado nociva.")
...
def registrar_muestra_db(datos, db_path=DB_PATH):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()

    dictamenes_json = json.dumps(datos.get("dictamenes", []))
    ...
    """, (
        ...
        datos.get("na2o"), datos.get("k2o"), datos.get("p2o5"), datos.get("pb"), datos.get("cd"), datos.get("as_ppm"),
        datos.get("drx", "Calcita"), datos.get("petrografia", "Micrítica de grano fino"),
        datos["loi"], datos["res_insol"], datos["alcalis"],
        datos["lsf"], datos["sm"], datos["am"], datos["c3s"], datos["c2s"], datos["c3a"], datos["c4af"],
        datos["estado_eval"], datos.get("archivo_fuente", "Manual"),
        datos.get("fecha_registro") or datetime.now().strftime("%Y-%m-%d %H:%M"),
        dictamenes_json,
        extras.get("pn"), extras.get("blancura"), extras.get("tamano_particula"), extras.get("humedad"),
        extras.get("cao_disponible"), extras.get("cao_reactivo"), extras.get("resistencia"), extras.get("absorcion")
    ))
    conn.commit()
    conn.close()


def obtener_muestras_db(db_path=DB_PATH):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM muestras ORDER BY fecha_registro DESC")
    ...
    for f in filas:
        ...
        try:
            d["dictamenes"] = json.loads(d["dictamenes_json"]) if d.get("dictamenes_json") else []
        except:
            d["dictamenes"] = []
        out.append(d)
    return out
...
def borrar_muestra_db(id_muestra, db_path=DB_PATH):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM muestras WHERE id_muestra = ?", (id_muestra,))
    conn.commit()
    conn.close()
