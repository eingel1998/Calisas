...
// Uso: bun scripts/crear-usuario.ts correo@ejemplo.com claveSegura123 ["Nombre"]
import { auth } from '../server/utils/auth'

async function main() {
  const [email, password, name] = process.argv.slice(2)
  if (!email || !password) {
    console.error('Uso: bun scripts/crear-usuario.ts correo@ejemplo.com claveSegura123 ["Nombre"]')
    process.exit(1)
  }
